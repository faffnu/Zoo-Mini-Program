<template>
  <div>
		...
		<div class="card" style="margin-bottom: 5px">
			<el-input v-model="data.search.name" prefix-icon="Search" style="width: 240px; margin-right: 10px; margin-bottom: 3px" placeholder="请输入分类名称"></el-input>
			<el-button type="info" plain @click="load">查询</el-button>
			<el-button type="warning" plain style="margin-right: 10px" @click="reset">重置</el-button>

			<el-upload :action="'http://localhost:9090/type/upload?token=' + data.token" style="display: inline-block" :show-file-list="false" :on-success="impSuccessUpload">
    ...
    <el-dialog title="请填写信息" v-model="data.formVisible" width="50%" :close-on-click-modal="false" destroy-on-close>
      <el-form :model="data.form" label-width="80px"  style="padding: 20px 30px">
				...
				<el-form-item label="分类描述" prop="content">
					<el-input type="textarea" v-model="data.form.content" placeholder="请输入分类描述" ></el-input>
				</el-form-item>
      </el-form>
      <template #footer>
...
// 导出
const exp = () => {
	location.href = 'http://localhost:9090/type/export?token=' + data.token
}

//下载模板
const download = () => {
  location.href = 'http://localhost:9090/type/download?token=' + data.token
}
