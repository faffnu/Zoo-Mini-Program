<template>
  <div>
		...
		<div class="card" style="margin-bottom: 5px">
			...
			<el-input v-model="data.search.comment" prefix-icon="Search" style="width: 240px; margin-right: 10px; margin-bottom: 3px" placeholder="请输入评论"></el-input>
    ...
    <el-dialog title="请填写信息" v-model="data.formVisible" width="40%" :close-on-click-modal="false" destroy-on-close>
      <el-form :model="data.form" label-width="80px"  style="padding: 20px 30px">
				...
				<el-form-item label="评论" prop="comment">
					<el-select style="width: 100%" v-model="data.form.commentId">
						<el-option v-for="item in data.commentData" :value="item.id" :label="item.content" :key="item.id"></el-option>
					</el-select>
				</el-form-item>
      </el-form>
      <template #footer>
...
// 导出
const exp = () => {
	location.href = 'http://localhost:9090/commentApprove/export?token=' + data.token
}
// 加载用户
const loadUser = () => {
